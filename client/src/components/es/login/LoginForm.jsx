...
export default function LoginForm() {
    ...
    return (
        <form className="flex flex-col gap-8" onSubmit={handleSignIn}>
            <button type={"button"} className={`sm:hidden flex items-center gap-1 w-fit text-white bg-primary hover:bg-primary-2 transition-colors py-1 px-4 rounded-full`} onClick={() => router.back()}>
            ...
            <div className="flex flex-col gap-5">
                ...
                <div className="flex justify-end lazy-load-3">
                    <Link className={`w-fit ${darkMode ? 'text-dark-text hover:text-white' : 'hover:text-light-main'} transition-colors`} href={`/recover-password`}>Recuperar contraseña</Link>
                </div>
            </div>
