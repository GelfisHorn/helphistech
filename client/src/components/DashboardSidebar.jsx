...
export default function DashboardSidebar() {
    ...
    return (
        <div className={`flex flex-col justify-between gap-10 w-[3.5rem] lg:w-[17rem] border-r ${darkMode ? 'border-neutral-900' : 'border-neutral-200'}`} style={{height: 'calc(100vh - 3.5rem)'}}>
            <div className="flex flex-col gap-5">
                <div className={`p-2 border-b ${darkMode ? 'border-neutral-900' : 'border-neutral-200'}`}>
                    <div className={`flex justify-center lg:justify-start items-center gap-3 ${darkMode ? 'bg-neutral-900' : 'bg-neutral-100 shadow-sm'} py-3 px-3 rounded-lg`}>
                        <div>
                            {auth.profile_img ? (
            ...
            <div className={`border-t ${darkMode ? 'border-neutral-900' : 'border-neutral-200'} pb-5 pt-5`}>
    ...
    return (
        permissions.indexOf(auth.permissions) != -1 ? (
            <div className={`flex flex-col gap-2`}>
                <div className={`${darkMode ? 'text-zinc-400' : 'text-zinc-600'} hidden lg:block uppercase font-medium text-sm ml-6`}>{title}</div>
...
function SidebarItem({ children, icon, href, permissions, hrefTargetBlank }) {
    ...
    return (
        <Link href={href} target={hrefTargetBlank ? "_blank" : null} className={`${permissions.indexOf(auth.permissions) != -1 ? 'flex' : 'hidden'} flex justify-center`}>
            <button className={`${darkMode ? 'text-dark-text' : 'text-black'} ${actualRoute == href ? "text-primary" : null} flex items-center lg:justify-start gap-[0.85rem] lg:gap-5 lg:rounded-md py-2 w-full hover:text-primary uppercase font-medium text-left transition-colors select-none`}>
                <div className={`w-1 h-full ${actualRoute == href ? "bg-primary" : "bg-transparent"} rounded-r-md transition-colors`}></div>
