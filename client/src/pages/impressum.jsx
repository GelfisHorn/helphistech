...
export default function Impressum() {
    ...
    async function getImpressumData() {
        setLoading(true);
        try {
            const { data } = await axios.post(`/api/impressum`, { language });
            setData(data);
        } catch (error) {
    ...
    return (
        <Layout title={data.title || "Impressum"} lang={language} metaDesc={data.meta || ""} styles={{ backgroundColor: darkMode ? "#080808" : "#F6F6F6" }}>
